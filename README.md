...
接続状態が変化したとき、RGB LEDが約750ms点灯します。起動直後は未接続として赤く点灯します。CentralでBLEプロファイルを切り換えたときは一時的な切断状態を表示せず、切り換え先の状態が確定してから表示します。

| 色 | Central（右手側） | Peripheral（左手側） |
| --- | --- | --- |
| 青 | ホストに接続済み | Centralに接続済み |
| 赤 | 未接続（Advertisingを含む） | 未接続（Advertisingを含む） |

> [!info]
> RMK v0.9へでbond(ペアリング)状態を取得できなくなったため、ペアリング待ちと再接続待ちは区別せず、Advertising中は一律赤で表示する実装へ変更しました。bond状態が取れるようになったら戻します。
